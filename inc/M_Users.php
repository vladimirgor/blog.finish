...
{	
    ...
    public function Signing($first_name, $last_name, $login, $password, $remember = true)
    {
        ...
        if ($user == null){
            ...
            //  записываем нового пользователя в users.

            $result = $this->msql->Insert('users',['login'=>$login,'first_name'=> $first_name,
                'last_name'=> $last_name, 'password'=>md5($password)]);

        } else return false; //  user exists already
        ...
        // открываем сессию и запоминаем SID
        $this->sid = $this->OpenSession($result);
        return true;
    }
	//
	// cleaning
